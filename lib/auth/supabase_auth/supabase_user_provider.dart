...
class MIDASSupabaseSupabaseUser extends BaseAuthUser {
  MIDASSupabaseSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> mIDASSupabaseSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MIDASSupabaseSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
